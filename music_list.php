...
<div class="col-lg-12">
	<div class="d-flex justify-content-between align-items-center w-100">
		<div class="form-group" style="width:calc(50%) ">
			<div class="input-group">
              <input type="search" id="filte" class="form-control form-control-sm" placeholder="Search music using keyword">
	...
	<div class="row" id="music-list">
		<?php 
			$sql = "SELECT u.*,g.genre FROM uploads u inner join genres g on g.id = u.genre_id order by u.title asc";
			$musics = mysqli_query($conn,$sql);
		...
		<div class="card bg-black music-item my-2 mx-1" data-id="<?php echo $row['id'] ?>" data-upath="<?php echo $row['upath'] ?>" style="width:15vw;cursor:pointer;">
			<div class="card-img-top flex-w-100 position-relative py-2 px-3">
				<?php if($_SESSION['users']['type']  == 1 || $_SESSION['users']['id'] == $row['user_id']): ?>
				...
				<!-- <span class="position-absolute" style="bottom:.5em;left:.5em;z-index: 2"><div class="btn bg-green rounded-circle d-flex justify-content-center align-items-center" style="width: 2rem;height: 2rem;cursor: pointer;"><i class="fa fa-play"></i></div></span> -->
			     <a href="try.php?id=<?php echo $row['id'] ?>"> 
				
				<img src="<?php echo $row['cover_image'] ?>" class="card-img-top"  style="object-fit: cover;max-width: 100%;height:26vh" alt="music Cover">
			</div>
			<div class="card-body border-top border-primary" style="min-height:20vh">
				...
				<p class="card-text truncate text-white"><small><?php echo $row['description']; ?></small></p>
			</div>
		</a>
		</div>
		<?php endwhile; ?>
